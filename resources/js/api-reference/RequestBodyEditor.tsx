...
export function RequestBodyEditor({
    ...
    const fieldSchema = useMemo(() => normalizeFieldSchema(schema, components, new Set()), [components, schema]);
    const parsed = parseJson(value);

    if (fieldSchema?.kind !== "object" || !isRecord(parsed)) {
        return (
            <FormFieldFrame id={`${idPrefix}-request-body`} label="JSON body" required={required} error={error}>
                {(controlProps) => (
                    <Textarea
                        {...controlProps}
                        value={value}
    ...
    if (schema.kind === "object") {
        return (
            <fieldset className="min-w-0 rounded-lt-sm border border-lt-border p-3 @xl:col-span-2">
                ...
                {schema.description ? <p className="mb-3 text-xs text-lt-muted-fg">{schema.description}</p> : null}
                ...
                {items.map((item, index) => (
                    <div key={index} className="flex min-w-0 items-start gap-3">
                        ...
                        <Button
                            ...
                            onClick={() => onChange(path, items.filter((_item, itemIndex) => itemIndex !== index))}
                        >
                            Remove
                        </Button>
                    </div>
...
function normalizeFieldSchema(schema: unknown, components: unknown, visitedRefs: Set<string>): FieldSchema | null {
    if (!isRecord(schema)) {
        return null;
    }
    ...
    if (typeof schema.$ref === "string") {
        ...
        const normalized = normalizeFieldSchema(referenced, components, new Set([...visitedRefs, schema.$ref]));
    ...
    const enumValues = Array.isArray(schema.enum) && schema.enum.every(isScalar)
        ? schema.enum
        : [];
...
function mergeObjects(
    ...
    return {
        kind: "object",
        description: stringValue(schema.description) ?? objects.find((object) => object.description)?.description ?? null,
        initialValue: exampleFromSchema(schema, components),
...
function updateValue(root: Record<string, unknown>, path: Path, value: unknown): Record<string, unknown> {
...
function containerValue(container: Record<string, unknown> | unknown[], key: string | number): unknown {
...
function pathLabel(path: Path): string {
    return path.reduce<string>(
        (label, segment) =>
            typeof segment === "number" ? `${label}[${segment}]` : label === "" ? segment : `${label}.${segment}`,
        "",
    );
}
...
function inputType(schema: Extract<FieldSchema, { kind: "string" | "number" | "integer" | "boolean" }>): React.HTMLInputTypeAttribute {
...
function inputStep(schema: Extract<FieldSchema, { kind: "string" | "number" | "integer" | "boolean" }>): number | "any" | undefined {
