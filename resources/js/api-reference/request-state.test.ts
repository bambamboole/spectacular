...
import { initialRequestValues, isJsonMediaType, jsonRequestContracts, parameterKey } from "./request-state";
...
describe("request JSON media types", () => {
    ...
    it("preserves the order of JSON-compatible request contracts", () => {
        ...
        expect(jsonRequestContracts(operation({ requests: [form, problem, json] }))).toEqual([problem, json]);
    });
});
...
describe("initialRequestValues", () => {
    it("prefers direct parameter examples, then schema examples, defaults, enums, and finally an empty value", () => {
        ...
        expect(initialRequestValues(operation({ paramGroups: [{ location: "query", params }] })).parameters).toEqual({
            "query:direct": "42",
    ...
    it("selects the first JSON-compatible contract and pretty-prints its explicit example", () => {
        const values = initialRequestValues(
            operation({
                requests: [
                    requestContract({ mediaType: "multipart/form-data", examples: [{ name: null, summary: null, value: "ignored" }] }),
                    requestContract({
                        mediaType: "application/problem+json",
                        examples: [{ name: "problem", summary: null, value: { title: "Invalid", status: 422 } }],
                    }),
                    requestContract({ mediaType: "application/json", examples: [{ name: null, summary: null, value: { ignored: true } }] }),
                ],
            }),
        );
    ...
    it("pretty-prints a schema-derived example with component references", () => {
        const values = initialRequestValues(
            operation({ requests: [requestContract({ schema: { $ref: "#/components/schemas/Widget" } })] }),
            {
                schemas: {
                    Widget: {
                        type: "object",
    ...
    it("returns no selected body when only non-JSON contracts exist", () => {
        expect(
            initialRequestValues(operation({ requests: [requestContract({ mediaType: "multipart/form-data" })] })),
